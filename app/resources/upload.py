...
def upload_file():
    ...
    upload_path = BaseConfig.UPLOAD_FOLDER + "/" + date_today.strftime("%Y/%m/%d")
    ...
    if request.method == "GET":
        ...
        if request.args.get("file") == "":
            return (
                json.dumps(
                    {
                        "status": "404",
                        "message": "vui lòng truyền vào link file cần chuyển đổi",
                    }
                ),
                404,
            )
        ...
        if file_url.split(".")[-1] not in BaseConfig.ALLOWED_EXTENSIONS:
            content_disposition = requests.get(file_url).headers["Content-Disposition"]
            value, params = cgi.parse_header(content_disposition)
            name_file = params["filename"]
            if re.search(r"\s", name_file):
                name_ext = name_file.split(".")[-1]
                name_edit = (
                    (name_file[: name_file.rindex(".")]).replace(" ", "-")
                    + "."
                    + name_ext
                )
            ...
            try:
                ...
                return (
                    json.dumps(
                        {
                            "status": "500",
                            "message": u"Lỗi hệ thống, không thể download file",
                        }
                    ),
                    500,
                )
...
def download_file(filename):
    ...
    return send_from_directory(
        directory=BaseConfig.CONVERT_FOLDER, filename=filename, as_attachment=True
    )
